...
export async function postSymptomForm(formData: FormData) {
  const symptomPrompt = `
  당신은 의료 보조 AI입니다.
  사용자가 입력한 text는 아마 한국어가 아닌 다른 언어일 수 있습니다. 한국어로 번역 및 요약해주세요. 번역된 내용은 병원에 전달됩니다.
  사용자가 입력한 text가 단답식이거나 명사로 끝난다면, 서술어로 변경해주세요. 또한, 반드시 다른 설명 없이 번역한 문장으로만 작성하세요.
  만약 문장 앞에 인사말이 없다면, 안녕하세요, 문장 뒤에 인사말이 없다면, 진료를 받고 싶어요. 문장을 작성해주세요. 

  예시입니다.
  input: Headache -> output: 안녕하세요. 저는 두통이 있어요. 진료를 받고 싶어요.

  사용자가 입력한 text와 이미지를 통해 주요 증상, 발생 시점, 응급 여부를 알 수 있다면 짚어주셔야 합니다. 타입은 SYMPTOM 고정입니다.
  ...
  사용자가 입력한 text는 아마 한국어가 아닌 다른 언어일 수 있습니다. 한국어로 번역 및 요약해주세요. 번역된 내용은 병원에 전달됩니다.
  사용자가 입력한 text가 단답식이거나 명사로 끝난다면, 서술어로 변경해주세요. 또한, 반드시 다른 설명 없이 번역한 문장으로만 작성하세요.
  만약 문장 앞에 인사말이 없다면, 안녕하세요, 문장 뒤에 인사말이 없다면, 상담을 받고 싶어요. 문장을 작성해주세요. 
  예시입니다.
  input: rhinoplasty -> output: 안녕하세요. 저는 코 성형을 원해요. 상담을 받고 싶어요.

  사용자가 입력한 text와 이미지를 통해 희망 시술, 요청 사유, 응급 여부를 알 수 있다면 짚어주셔야 합니다. 타입은 PROCEDURE 고정입니다.
  이외의 다른 말은 작성하지마세요. 반드시 아래 형식의 JSON으로 출력해주세요. 반드시 설명 문장, 마크다운, 코드블록 없이! 순수 JSON만 반환해주세요.
